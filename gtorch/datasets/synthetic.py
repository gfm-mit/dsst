...
class SeqDataset(torch.utils.data.Dataset):
    def __init__(self, test_split=False):
      ...
      self.files = pd.DataFrame([
        ...
        np.random.normal(size=self.N),
        np.random.normal(size=self.N),
      ], index="one two three four five six".split()).T
      ...
      self.files["label"] = np.random.uniform(size=self.N) < scipy.special.expit(logit)

    def __getitem__(self, index):
      row = self.files.iloc[index]
      x = torch.Tensor(row.iloc[:6].astype(float).values[:, np.newaxis])
      y = torch.LongTensor([row.iloc[6]])
      if self.test_split:
        return x, y, index
      else:
        return x, y
