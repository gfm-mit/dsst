...
def plot_roc(roc):
  ...
  plt.plot(roc.y_hat, 1 - roc.cost_empirical / optimal, color=empirical_color, label="empirical")
  optimal = np.minimum(roc.y_logistic, 1 - roc.y_logistic)
  plt.plot(roc.y_logistic, 1 - roc.cost_logistic / optimal, color=logistic_color, label="logistic")
  optimal = np.minimum(roc.y_convex, 1 - roc.y_convex)
  plt.plot(roc.y_convex, 1 - roc.cost_convex / optimal, color=convex_color, label="convex")
  plt.xscale('logit')
  plt.legend()
  plt.xlabel('skew')
  plt.ylabel('cost')