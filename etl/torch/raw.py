...
class SeqDataset(torch.utils.data.Dataset):
    def __init__(self, metadata, test_split=False, device=None, task=None, columns="t v_mag2 a_mag2 dv_mag2 cw j_mag2"):
        self.device = device
    ...
    def cache_miss(self, indices, device=None):
        ...
        padded = np.zeros([len(indices), max_len, len(self.columns)])
...
def get_loaders(device=None, task=None, batch_size=1000, shuffle=True, columns="t v_mag2 a_mag2 dv_mag2 cw j_mag2"):
  ...
  train_data = SeqDataset(labels.loc["train"], device=device, task=task, columns=columns)
  val_data = SeqDataset(labels.loc["val1"], device=device, task=task, columns=columns)
  calibration_data = SeqDataset(labels.loc["val1"], device=device, test_split=True, task=task, columns=columns)
  test_data = SeqDataset(labels.loc["val2"], device=device, test_split=True, task=task, columns=columns)
